if "e_compact" in types:
    types = list(map(lambda t: t.replace("e_compact", "find=e_compact"), types))
        ...
        for prop_res in resource_desc:
            for resource_value in prop_res["resources"]:
                if not resource_value["resource"] == "core":
                    raise Exception(
                        "# ADMISSION RULE> Error: If type e_compact is given, only core can be used as resource type."
                    )
