def estimate_job_nb_resources_in_e_spread(
...
if "e_spread" in types:
    types = list(map(lambda t: t.replace("e_spread", "find=e_spread"), types))
        ...
        for prop_res in resource_desc:
            for resource_value in prop_res["resources"]:
                if not resource_value["resource"] == "core":
                    raise Exception(
                        "# ADMISSION RULE> Error: If type e_spread is given, only core can be used as resource type."
                    )

    if (
        estimate_job_nb_resources_in_e_spread(
            session, config, resource_request, properties
        )[0][0]
        < 0
    ):
        raise Exception(
            "# ADMISSION RULE> There are not enough resources for your request using the e_spread method"
        )
